...
- **Key pinning exists, but is opt-in and build-time.** Build with
  ...
  renewal: `tools/spki_pin.py --check-baseline`). On `ip65-onchip` a pin is
  the only certificate check there is. Issue #155.
...
$4F8C-$5FFF  CRYPTO_OVERLAY      4,212 B; holds TLS_CODE + CRYPTO_AUX_CODE
                                 + HTTP_AUX_CODE + HTTP_AUX_CODE2 (that
                                 last one rode the CRYPTO_RESIDENT tail
                                 until the v0.14.0 bump moved it here in
                                 cfg/c64-https-ip65.cfg — check the cfg,
                                 not this line, on any other tree)
