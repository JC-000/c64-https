...
.ifdef BACKEND_UCI
        ...
        ; PROVISIONAL fallback definitions — laneE-bodysink owns
        ; http_body_total (24-bit LE body length) and http_body_sink
        ; (flag: body was sunk to REU). Until its definitions land this
        ; block makes the branch link standalone. Merge resolution:
        ; build with -D VIEWER_EXTERNAL_SINK=1 (or delete this block);
        ; a 'Duplicate external identifier' from ld65 IS the reminder.
        .ifdef VIEWER_EXTERNAL_SINK
...
vw_tbi:         .res 1                  ; row-table byte index
        ...
        .ifndef VIEWER_EXTERNAL_SINK
http_body_total: .res 3                 ; PROVISIONAL — see header
http_body_sink:  .res 1                 ; PROVISIONAL — see header
        .endif
