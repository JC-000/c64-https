...
; via tools/integration/gen_p384_overlay_equates.sh, regenerated by the
; Makefile whenever either overlay labels file changes (Phase 5 Fix C).
;
; Both overlays load at $4200 (CRYPTO_OVERLAY) and their resident DATA
; lives at $C000+ (TCP_BUF range).  The build-time .assert pins below
; catch drift if either invariant breaks (e.g. an overlay cfg restructure
...
; equates file violates either range, ld65 / ca65 won't catch it; these
; .asserts will.
.assert sha384_init = $4200, error, "sha384_init must be at the overlay slot start ($4200)"
.assert sha384_update >= $4200 .and sha384_update < $6000, error, "sha384_update outside CRYPTO_OVERLAY"
.assert sha384_final  >= $4200 .and sha384_final  < $6000, error, "sha384_final outside CRYPTO_OVERLAY"
.assert sha384_digest >= $C000 .and sha384_digest < $D000, error, "sha384_digest outside overlay-resident DATA range"
.assert ecdsa_verify_384 >= $4200 .and ecdsa_verify_384 < $6000, error, "ecdsa_verify_384 outside CRYPTO_OVERLAY"
.assert ecdsa_inputs_384 >= $C000 .and ecdsa_inputs_384 < $D000, error, "ecdsa_inputs_384 outside overlay-resident DATA range"
