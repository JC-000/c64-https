...
; x25519 named group (not in constants.asm)
...
; Output: tls_hs_buf contains complete handshake message
;         tls_hs_len = total length (16-bit)
...
.skip_sni:
        ...
        sec
        sbc #47
        sta tls_hs_buf+46              ; low byte of extensions length
        lda #0
        sta tls_hs_buf+45              ; high byte (always 0, msg < 256)

        ; handshake length at [1-3] = Y - 4 (24-bit big-endian)
        tya
        sec
        sbc #4
        sta tls_hs_buf+3               ; low byte
        lda #0
        sta tls_hs_buf+2               ; mid byte
        sta tls_hs_buf+1               ; high byte
...
;
; Input:  tls_hs_buf contains raw ServerHello (including handshake header)
;         tls_hs_len = length
...
.sh_no_session_id:
        ...
        lda tls_hs_buf,y               ; high byte
        sta zp_tmp2                     ; extensions remaining (high)
        iny
        lda tls_hs_buf,y               ; low byte
...
; Extension tracking flags (inline data)
.sh_found_ver: !byte 0
.sh_found_ks:  !byte 0
...
;
; Input:  tls_hs_buf contains decrypted handshake message
...
; =============================================================================
tls_hostname:       !fill 64, 0
tls_hostname_len:   !byte 0
