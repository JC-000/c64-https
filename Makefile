...
#       -> a65fff60...   (correct)
#
# HTTPS_SNI= feeds two consumers: the string travels in build/https_host.inc
# below (which invalidates boot.o), and a non-empty value also adds
# -D HTTPS_SNI_OVERRIDE=1, which src/http.s reads. Nothing invalidated
# http.o, so the image carried the SNI string and never ran the override —
# indistinguishable from not passing the flag. The PRG hash changed and the
# name grepped out of the image, so only a diff against a cleaned build could
# see it; it cost a full false negative on issue #141 (whose narrower fix,
# adding build/http.o to the https_host.inc rm below, this subsumes).
#
# The mechanism is the one issue #128 already proved out for the target
# strings, widened to the flag set: write the flags to a stamp file, content-
# compare it during Makefile EXPANSION, and on a change DELETE the objects
# and the PRG. Parse time, not a recipe, and absence, not a timestamp —
# both for the reasons spelled out in the https_host.inc block below, which
# is the load-bearing part of this design and is worth reading before
# touching either block.
#
# What is stamped is the fully expanded ca65 and ld65 command lines, not a
...
#
# NOT stamped, deliberately: HTTPS_HOST / HTTPS_PATH / HTTPS_SNI's string
# values. They reach ca65 through build/https_host.inc, which invalidates
# boot.o + http.o instead of the whole tree — issue #128's no-`make clean`
# ergonomic, which certificate pinning (#155) is about to depend on. A
# non-empty HTTPS_SNI does change CA65FLAGS, so the presence of the override
# is stamped even though the name in it is not.
#
# Cost of a false positive here is one full rebuild (~0.4 s); cost of a false
# negative is the incident above. tools/test_build_flags_stamp.py pins both
# directions.
#
# One consequence worth knowing: CA65FLAGS carries two $(abspath ...)
# `.incbin` roots, so the stamp is absolute-path-bearing and moving a
# checkout forces one rebuild. That is the honest answer — a moved tree IS a
# different `.incbin` root, which is the bug issue #116 chased through agent
...
FLAGS_STAMP_BODY := printf '%s\n' \
    ...
    'A change to any line deletes every object and the PRG at parse time.' \
    'Target strings (HTTPS_HOST/HTTPS_PATH/HTTPS_SNI) live in the separate' \
    'build/https_host.inc stamp, which invalidates less. See the Makefile.' \
    'CA65=$(CA65)' \
    'LD65=$(LD65)' \
    'BACKEND=$(BACKEND)' \
    'CA65FLAGS=$(CA65FLAGS)' \
...
# relationship for anyone reading the graph, and keeps the stamp from being
# a file the graph does not know about. A NORMAL prerequisite would make
# every object depend on the stamp's mtime — a 1-second-resolution
# comparison, which is exactly what this design exists not to rely on.
$(ALL_OBJS): | $(FLAGS_STAMP)
endif

# Recovery rule for the order-only edge. Normally unreachable: the block
# above writes the stamp during parse, so by the time make looks the file is
# there (and if it had been deleted by hand, the compare above already
# treated that as a change and cleared the objects). It exists so the edge
# can never fail a build with "no rule to make target" — the failure mode
# `make clean all` still has for build/https_host.inc, which predates this
# and is left alone.
$(FLAGS_STAMP):
	@mkdir -p build
	@$(FLAGS_STAMP_BODY) > $@
...
# parse-time invalidation below closes it. It is retired for BACKEND= and
# the rest of the flag set too, since build/flags.stamp above gives them the
# same hook — but the two stamps invalidate at different grains on purpose,
# and only this one is cheap enough to fire on every retarget.
...
# consumer of an HTTPS_SNI= change: it carries the .ifdef
# HTTPS_SNI_OVERRIDE prologue that copies https_sni_override into
# tls_hostname. It does NOT .include the generated header — the string
# itself is boot.s's, and http.s only .imports the label — so this edge is
# insurance, not the fix: what closed the mixed link is build/flags.stamp
# above, which moves on the empty -> non-empty transition because that
