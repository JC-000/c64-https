...
drops a symbol fails the link by name on both backends. Surface:
  ...
  - Byte accounting on ip65 (the tight one): LOADER went from 16 B free to
    ...
    that fix, measured), and is **32 B** after #239, which is the number the
...
## Known issues
    ...
    target:
      - inline in `CODE` took ip65's LOADER from the 21 B free it had
        *then* to **zero** — it fit exactly, so the *next* byte anyone
        ...
        instead, and LOADER measured **17 B** free (32 B after #239). The 4 B is
  ...
  - Region margins measured at #239 (they drift, so re-measure
    rather than cite): ip65 **32 B** LOADER, **56 B** NET_CODE tail
    (14 B with the wikipedia target), **51 B** CRYPTO_OVERLAY. UCI comb
...
default target strings already use), 145 B CRYPTO_RESIDENT, 51 B
CRYPTO_OVERLAY, 32 B LOADER, and a 43 B hole
below TABLES_BSS in
