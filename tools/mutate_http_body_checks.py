...
MUTANTS = [
    ...
    ("#226 R2: the close wait's extra exit always fires (`... or 'x'`: "
     ...
     "                                         read_tcp_state, closed_before)"
     " or 'x')"),
    # --- #247: the handshake-failure stop. Each is a way it could stop a
    # --- live handshake, or end the close wait over a live socket.
    ("#247: handshake_failed ignores tls_state",
     MODULE,
     "    if tls_state != TLS_STATE_ERROR:\n        return None",
     "    if False and tls_state != TLS_STATE_ERROR:\n        return None"),
    ("#247: handshake_failed fires after the body began",
     MODULE,
     "    if state.parse_state >= PARSE_STATE_BODY:\n        return None",
     "    if False and state.parse_state >= PARSE_STATE_BODY:\n"
     "        return None"),
    ("#247: handshake_failed trusts a ROM read",
     MODULE,
     "    if not shadow_ok:\n        return None\n    if tls_state",
     "    if False and not shadow_ok:\n        return None\n    if tls_state"),
    ("#247: the handshake stop does not wait for net_tcp_close",
     MODULE,
     "            if tcp == NET_TCP_CLOSED:",
     "            if True or tcp == NET_TCP_CLOSED:"),
    ("#247: close_confirmed believes a truthy non-string closed_before",
     MODULE,
     "    if isinstance(closed_before, str) and closed_before:",
     "    if closed_before:"),
    ("#247: the rig drops the tls_state reader (pre-#247 budget burn)",
     RIG,
     "                read_tls_state=read_tls_state)",
     "                read_tls_state=None)"),
    ("#247: the rig records closed_before for a stop past the body",
     RIG,
     "                if state.parse_state < PARSE_STATE_BODY:\n"
     "                    closed_before = why",
     "                if True:\n"
    ...
    ("#226 R3: poll_until never waits out its budget",
     ...
     "        if clock() >= deadline:\n            return None, lines",
     "        if True:\n            return None, lines"),
    ("#226 R4: poll_until clamps its budget to 10 s",
     MODULE,
     "    deadline = clock() + budget\n",
     "    deadline = clock() + min(budget, 10.0)\n"),
