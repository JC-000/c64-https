...
MUTANTS = [
    ...
    ("leak_sources ignores partial-only hits",
     'ev["leak_sources"] = sorted({h["eth_src"] for h in full_hits + partial_hits})',
     'ev["leak_sources"] = sorted({h["eth_src"] for h in full_hits})'),
    ("a stream hit is attributed to the capture's first frame's source",
     '                where.append({"kind": "stream", "eth_src": fmt_mac(src)})',
     '                where.append({"kind": "stream", '
     '"eth_src": fmt_mac(frames[0].eth_src)})'),
     ...
     *_off("    if recv_overflow and not stream_verified:")),
    ("check_net_counters excuses an overflow without a verified stream",
     "    if recv_overflow and not stream_verified:",
     "    if recv_overflow and stream_verified is None:"),
    ("check_net_counters never excuses an overflow",
     "    if recv_overflow and not stream_verified:",
     "    if recv_overflow:"),
    ...
    ("check_ip65_config_written compares only the last 3 bytes",
     "        if want is not None and bytes(got) != bytes(want):",
     "        if want is not None and bytes(got)[-3:] != bytes(want)[-3:]:"),
    ("check_ip65_config_written compares only the first 3 bytes",
     "        if want is not None and bytes(got) != bytes(want):",
     "        if want is not None and bytes(got)[:3] != bytes(want)[:3]:"),
    ("check_ip65_config_written compares byte multisets (order-blind)",
     "        if want is not None and bytes(got) != bytes(want):",
     "        if want is not None and sorted(bytes(got)) != sorted(bytes(want)):"),
    ("check_ip65_config_written accepts a byte-reversed value",
     "        if want is not None and bytes(got) != bytes(want):",
     "        if want is not None and bytes(got) != bytes(want) "
     "and bytes(got)[::-1] != bytes(want):"),
